#' export.msp.formatted.spectra.gc.ei
#'
...
export.msp.formatted.spectra.gc.ei <- function(
    ramclustObj = NULL,
    output.dir = NULL, 
    one.file = FALSE,
    one.file.name = "spectra.msp",
    mz.digits = 1
) {
  ...
  if(!one.file) {
    if(!dir.exists(paste0(output.dir, '/spectra/msp'))) {
      dir.exists(paste0(output.dir, '/spectra/msp/'))
    }
  }
  ion.mode <- "positive"
  ...
  for(i in 1:length(ramclustObj$cmpd)) {
    ...
    if(ms2) {
      ...
      spectrum <- data.frame(
        'mz' = round(ramclustObj$fmz[ions], mz.digits),
  ...
  if(one.file) {
    ...
    sink(paste0(output.dir, '/spectra/', one.file.name))
    cat(out)
    sink()
  } else {
    for(i in 1:length(out.list)) {
