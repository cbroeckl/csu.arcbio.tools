...
#' @param rt.sd numeric: multiplier for the cluster standard deviation for retention times.  used for defining the retention time window to consider for fragment ion retention time assignment. retention time window is +/-  max(min.rt.range, rt.sd*ramclustObj$clrtsd)
#' @param min.rt.range numeric: minimum retention time range (in seconds, for xcms data) to be considered. retention time window is +/-  max(min.rt.range, rt.sd*ramclustObj$clrtsd)
